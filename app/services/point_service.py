...
class PointsService:
    ...
    async def award_points(
        ...
        try:
            ...
            badges_unlocked = await self._check_and_award_badges(user_id, new_total_points)
            ...
            return {
                ...
                "message": f"Awarded {points} points! {len(badges_unlocked)} new badge(s) earned!" if badges_unlocked else f"Awarded {points} points!"
            }
    ...
    async def _update_user_stats(self, user_id: str, activity_type: str) -> None:
        ...
        try:
            ...
            if activity_type == "issue_reported":
                update_data["issues_reported"] = (user.get("issues_reported", 0) or 0) + 1
            
            elif activity_type == "cleanup_verified":
                update_data["tasks_completed"] = (user.get("tasks_completed", 0) or 0) + 1
                # Update streak
                last_volunteer_date = user.get("last_volunteer_date")
                current_streak = user.get("volunteer_streak", 0) or 0
                
                if last_volunteer_date:
                    last_date = datetime.strptime(last_volunteer_date, "%Y-%m-%d").date() if isinstance(last_volunteer_date, str) else last_volunteer_date
    ...
    async def _check_and_award_badges(self, user_id: str, current_points: int) -> list:
        ...
        try:
            ...
            badges_response = self.supabase.table("badge_definitions").select("*").execute()
            
            if not badges_response.data:
                return badges_unlocked
            
            # Get user's current badges (to avoid duplicates)
            user_badges_response = self.supabase.table("user_badges").select("badge_type").eq("user_id", user_id).execute()
            user_badge_types = set(b["badge_type"] for b in (user_badges_response.data or []))
            ...
            for badge_def in badges_response.data:
                ...
                condition_met = await self._check_badge_condition(user, condition, user_id)
    ...
    async def _check_badge_condition(self, user: Dict, condition: Dict, user_id: str) -> bool:
        """
        ...
        try:
            ...
            # CLEANUP COMPLETION
                ...
                activity_response = self.supabase.table("user_activity_log").select("id").eq("user_id", user_id).eq("activity_type", "cleanup_verified").execute()
            ...
            # GROUP CLEANUPS
                ...
                volunteers_response = self.supabase.table("volunteers").select("id").eq("user_id", user_id).eq("solo_work", False).eq("verified", True).execute()
            ...
            # BADGES COUNT (for Legend badge)
                ...
                badges_response = self.supabase.table("user_badges").select("id").eq("user_id", user_id).eq("is_permanent", True).execute()
            ...
            # WEEKLY CONDITIONS
                ...
                activity_response = self.supabase.table("user_activity_log").select("points_earned").eq("user_id", user_id).gte("activity_date", week_start.isoformat()).execute()
                ...
                week_start = self._get_week_start_date()
                activity_response = self.supabase.table("user_activity_log").select("id").eq("user_id", user_id).eq("activity_type", "cleanup_verified").gte("activity_date", week_start.isoformat()).execute()
                count = len(activity_response.data or [])
                return count >= threshold
    ...
    async def _invalidate_cache(self, user_id: str) -> None:
        ...
        try:
            ...
            self.supabase.table("leaderboard_cache").delete().eq("user_id", user_id).execute()
        ...
        try:
            ...
            for user_id in user_ids:
                ...
                if weekly_points >= 100:
                    ...
                    existing = self.supabase.table("user_badges").select("id").eq("user_id", user_id).eq("badge_type", "momentum").gte("earned_at", week_start.isoformat()).execute()
                ...
                if cleanup_count >= 3:
                    # Award on_fire badge
                    existing = self.supabase.table("user_badges").select("id").eq("user_id", user_id).eq("badge_type", "on_fire").gte("earned_at", week_start.isoformat()).execute()
                    if not existing.data:
                        self.supabase.table("user_badges").insert({
                            "user_id": user_id,
                            "badge_type": "on_fire",
