...
class LeaderboardService:
    ...
    async def _calculate_rankings(
        self, leaderboard_type: str, location_type: str, location_value: Optional[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if location_type == "global":
                ...
                response = (
                    ...
                    .select("*")
                    .eq("admin_region", location_value)
                    .execute()
                )
                return response.data or []
                ...
                if not location_value:
                    raise ValueError(
                        "Coordinates required for community filter (lat,lng,radius)"
                    )
    ...
    async def _get_cache(self, cache_key: str) -> Optional[List[Dict]]:
        ...
        try:
            response = (
                self.supabase.table("leaderboard_cache")
                .select("*")
                .eq("id", cache_key)
                .execute()
            )
    ...
    async def invalidate_cache(self, user_id: str) -> None:
        ...
        try:
            ...
            self.supabase.table("leaderboard_cache").delete().gte(
                "cached_at",
                (
                    datetime.utcnow() - timedelta(minutes=self.CACHE_TTL_MINUTES)
                ).isoformat(),
    ...
    async def _award_rising_star_badges(self) -> int:
        ...
        try:
            ...
            users_response = (
                self.supabase.table("users").select("id,username").execute()
            )
                    ...
                    self.supabase.table("user_badges").insert(
                        {
                            ...
                            "metadata": {
                                ...
                                "weekly_rank": top_10.index(user) + 1,
                            },
                        }
                    ).execute()
                    badges_count += 1
