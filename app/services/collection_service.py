...
class CollectionsService:
    ...
    async def create_destination(
        ...
        try:
            ...
            response = self.supabase.table("destinations").insert(destination_data).execute()
    ...
    async def assign_destination_to_issue(
        ...
        try:
            ...
            dest_response = self.supabase.table("destinations").select("*").eq("id", destination_id).execute()
    ...
    async def start_collection(
        ...
        try:
            ...
            if not issue.get("destination_id"):
                raise ValueError("Issue must have a destination assigned before collection")
            
            # Check if collector already has active collection for this issue
            existing = self.supabase.table("collections").select("id").eq("user_id", user_id).eq("issue_id", issue_id).eq("status", "in_progress").execute()
            ...
            response = self.supabase.table("collections").insert(collection_data).execute()
    ...
    async def submit_collection(
        ...
        try:
            ...
            col_response = self.supabase.table("collections").select("*").eq("collected_by_user_id", user_id).eq("issue_id", issue_id).eq("status", "in_progress").execute()
            ...
            photo_url = await CloudinaryHelper.upload_photo(photo_bytes, folder=f"tankas-collections/{issue_id}")
            ...
            self.supabase.table("collections").update(update_data).eq("id", collection["id"]).execute()
    ...
    async def verify_delivery(
        ...
        try:
            # Get collection record
            col_response = self.supabase.table("collections").select("*").eq("id", collection_id).execute()
            ...
            # Upload proof photo
            proof_url = await CloudinaryHelper.upload_photo(photo_bytes, folder=f"tankas-deliveries/{collection_id}")
            ...
            collector_response = self.supabase.table("users").select("username, display_name").eq("id", collection["collected_by_user_id"]).execute()
            collector = collector_response.data[0] if collector_response.data else {}
            
            # Get destination info
            dest_response = self.supabase.table("destinations").select("name").eq("id", destination_id).execute()
            destination = dest_response.data[0] if dest_response.data else {}
            
            return {
                "collection_id": collection_id,
                "collector_id": collection["collected_by_user_id"],
                "collector_name": collector.get("display_name", collector.get("username", "Unknown")),
                "destination_name": destination.get("name", "Unknown"),
                "quantity_kg": collection.get("quantity_kg"),
                "points_earned": points_earned,
                "payment_amount": payment_amount,
                "status": "verified" if verified else "rejected",
                "message": f"Delivery verified! Collector earned {points_earned} points and GHS {payment_amount}" if verified else "Delivery rejected",
                "delivery_proof_url": proof_url
            }
    ...
    async def get_collection_by_id(self, collection_id: str) -> dict:
        ...
        try:
            ...
            issue_resp = self.supabase.table("issues").select("title, description, location").eq("id", collection["issue_id"]).execute()
            issue = issue_resp.data[0] if issue_resp.data else {}
            
            # Get destination info
            dest_resp = self.supabase.table("destinations").select("name, address").eq("id", collection["destination_id"]).execute()
            destination = dest_resp.data[0] if dest_resp.data else {}
            
            # Get collector info
            user_resp = self.supabase.table("users").select("username, display_name, avatar_url").eq("id", collection["collected_by_user_id"]).execute()
            ...
            return {
                ...
                "collector_id": collection["collected_by_user_id"],
                "collector_name": collector.get("display_name", collector.get("username")),
    ...
    async def get_collector_statistics(self, user_id: str) -> dict:
        ...
        """
        try:
            collections_resp = self.supabase.table("collections").select("*").eq("collected_by_user_id", user_id).execute()
            ...
            total_points = sum(int(c.get("quantity_kg") or 0 * self.POINTS_PER_KG) for c in verified_collections)
            
            return {
                "user_id": user_id,
                "total_collections": len(collections),
                "verified_collections": len(verified_collections),
                "pending_collections": len([c for c in collections if c["status"] in ["in_progress", "submitted"]]),
                "rejected_collections": len([c for c in collections if c["status"] == "rejected"]),
    ...
    async def get_destination_collections(
        ...
        """
        try:
            query = self.supabase.table("collections").select("*").eq("destination_id", destination_id)
            ...
            for col in collections:
                user_resp = self.supabase.table("users").select("username, display_name").eq("id", col["collected_by_user_id"]).execute()
                col["collector_name"] = user_resp.data[0]["display_name"] if user_resp.data else "Unknown"
    ...
    async def get_pending_verifications(self, destination_id: str) -> List[dict]:
        ...
        """
        try:
            response = self.supabase.table("collections").select("*").eq("destination_id", destination_id).eq("status", "submitted").order("submitted_at", desc=False).execute()
            ...
            for col in collections:
                ...
                issue_resp = self.supabase.table("issues").select("title").eq("id", col["issue_id"]).execute()
                col["issue_title"] = issue_resp.data[0]["title"] if issue_resp.data else "Unknown"
    ...
    async def delete_destination(self, destination_id: str) -> dict:
        ...
        try:
            ...
            active_resp = self.supabase.table("collections").select("id").eq("destination_id", destination_id).in_("status", ["in_progress", "submitted"]).execute()
            
            if active_resp.data:
                raise ValueError("Cannot delete destination with active collections")
            
            self.supabase.table("destinations").delete().eq("id", destination_id).execute()
